        ...
        detectedIntent = turnResult.intent;
          ...
          if (turnResult.nextAction !== 'generate_ai_response') {
            ...
            const suggestedActionPrefixes: Record<string, string> = {
              ...
              'Breakdown in details (MA)': '[SYSTEM CONTEXT: The user clicked "Breakdown in details (MA)". Your task is to EXPLAIN the most recent expression by showing how it was built up piece by piece — starting from the simplest building blocks and combining them into the full expression. This is an explanation of an existing expression — NOT a request to create a new one.\n\nHere is a CONCRETE EXAMPLE of the exact output format you must follow:\n\n**Expression:**\nIf(And(DateDiff(If(DateRe-Hired != "", DateRe-Hired, DateHired), Today()) / 365.25 >= 5, DateRe-Hired != ""), DateDiff(If(DateRe-Hired != "", DateRe-Hired, DateHired), Today()) / 365.25, If(And(DateDiff(DateHired, Today()) / 365.25 >= 5, DateRe-Hired == ""), DateDiff(DateHired, Today()) / 365.25, ""))\n\n**Step-by-Step:**\n\n**Step A — The basic years calculation:**\nDays between a date and today, converted to years:\nDateDiff(DateHired, Today()) / 365.25\n\n**Step B — Pick the right start date:**\nFor re-hired employees, we shouldn\'t count from their original hire date. So we first ask: does a re-hire date exist?\nIf(DateRe-Hired != "", DateRe-Hired, DateHired)\nThis says: "Use the re-hire date if there is one; otherwise fall back to the hire date."\n\n**Step C — Apply that to our years calculation:**\nPlug Step B into Step A as the starting date:\nDateDiff(If(DateRe-Hired != "", DateRe-Hired, DateHired), Today()) / 365.25\nNow we have a unified years-of-service number that automatically uses the right date.\n\n**Step D — The first branch: re-hired employees with 5+ years:**\nWe check two things together — they must have a re-hire date, AND their tenure from that date must be 5 or more years:\nAnd(...years from Step C... >= 5, DateRe-Hired != "")\nIf both are true, we return the years from their re-hire date. If not, we move to the next check.\n\n**Step E — The second branch: never re-hired but still 5+ years:**\nFor employees who were never re-hired, we check their original hire date tenure:\nIf(And(DateDiff(DateHired, Today()) / 365.25 >= 5, DateRe-Hired == ""), DateDiff(DateHired, Today()) / 365.25, "")\nThis says: "If they\'ve been here 5+ years and were never re-hired, show their tenure. Otherwise, show nothing."\n\n**Step F — Wrap it all together:**\nThe outer If from Step D wraps Step E as its else — so the two branches stack cleanly:\nIf(And(...Step C... >= 5, DateRe-Hired != ""), ...Step C..., ...Step E...)\n\nNow apply this same format to the most recent expression. Use EXACTLY this structure:\n\n**Expression:**\n<copy the full expression here, exactly as it appeared>\n\n**Step-by-Step:**\n\n**Step A — <Short descriptive title>:**\n<one or two sentences explaining what this step does in plain language, written for a non-technical user>\n<the sub-expression for this step, no code fences, just the raw expression text>\n\n**Step B — <Short descriptive title>:**\n<explanation; reference Step A if this step builds on it>\n<the sub-expression for this step>\n\n... (continue lettering through as many steps as the expression requires)\n\n**Step N — Wrap it all together:**\n<explanation of how all prior steps combine into the complete expression, referencing earlier steps by letter>\n<the full final expression>\n\nCRITICAL RULES:\n- Use letters (A, B, C...) not numbers.\n- Give each step a bold title that describes its purpose in plain English.\n- Show the sub-expression for each step on its own line below the explanation — no code fences, no backticks, just the raw expression text.\n- When a step builds on a prior step, say so explicitly (e.g., "Plug Step B into Step A").\n- Keep all explanations concise and non-technical — written for someone who understands the business logic but not the syntax.\n- Do NOT add any extra headers or commentary outside of the Step-by-Step format.\n- Do NOT use code fences or markdown code blocks.\n- This is an explanation of an existing expression. Do NOT apply expression-building validation rules to step labels.\n- CRITICAL: Do NOT output Python, JavaScript, or any other programming language code. Use ONLY the custom expression language (e.g. If(), DateDiff(), And(), etc.) and plain English text.]\n\n',
