        ...
        detectedIntent = turnResult.intent;
          ...
          if (turnResult.nextAction !== 'generate_ai_response') {
            ...
            const suggestedActionPrefixes: Record<string, string> = {
              ...
              'Breakdown in details (MA)': '[SYSTEM CONTEXT: The user clicked "Breakdown in details (MA)". Your task is to EXPLAIN the most recent expression by tracing how it was assembled, layer by layer, from the innermost value outward. This is an explanation of an existing expression — NOT a request to create a new one. Do NOT apply expression-building validation rules (such as requiring Value() wrappers) to step labels; the steps are illustrative traces for explanation only.\n\nHere is a CONCRETE EXAMPLE of the exact output format you must follow:\n\n**Expression:**\nIf(AnnualSalary == "", 0, Divide(Value(AnnualSalary), 12))\n\n**Step-by-Step:**\n1. AnnualSalary\n   → The source column holding each employee\'s annual salary figure.\n2. AnnualSalary == ""\n   → Identifies employees with no salary on record — a safeguard before any division is attempted.\n3. Value(AnnualSalary)\n   → Prepares the salary value for arithmetic so the result column supports sorting, filtering, and charting.\n4. Divide(Value(AnnualSalary), 12)\n   → Converts the annual figure to a monthly amount by dividing by 12.\n5. If(AnnualSalary == "", 0, Divide(Value(AnnualSalary), 12))\n   → Returns 0 for employees with no salary on file; otherwise returns the monthly salary.\n6. *Final:* If(AnnualSalary == "", 0, Divide(Value(AnnualSalary), 12))\n   → Produces a Numeric result — monthly salary for all employees, with 0 for those who have no annual salary recorded.\n\nNow apply this same format to the most recent expression. Use EXACTLY this structure:\n\n**Expression:**\n<copy the full expression here, exactly as it appeared>\n\n**Step-by-Step:**\n1. <the innermost column reference or literal value — no backticks, just the expression text>\n   → <one sentence: what this piece of data represents in business terms>\n2. <previous piece extended by exactly one additional layer or function>\n   → <one sentence: what this layer adds and why it is needed>\n... (continue, each step wrapping or extending the previous one by exactly one piece)\nN. *Final:* <the complete expression, written out in full>\n   → <one sentence: what the finished expression produces in the report>\n\nCRITICAL RULES:\n- Each step label shows the expression GROWING: every step includes everything from the previous step plus one new piece. Start from the innermost value and work outward.\n- EVERY step MUST have a → annotation line immediately after it. NEVER omit it.\n- Do NOT use code fences or markdown code blocks anywhere in your response.\n- Tone is professional and direct — written for a payroll administrator. Avoid phrases like "Now let\'s look at", "Think of it as", "Simply put", "This tells the system". Write annotations as direct business-language statements.\n- Each annotation is one sentence. Focus on the business reason behind the piece, not the technical syntax.\n- Do NOT include any additional headers, commentary, or explanatory paragraphs beyond Expression and Step-by-Step.]\n\n',
