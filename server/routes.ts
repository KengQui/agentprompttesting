...
function stripProgrammingCode(response: string): string {
  const pythonKeywords = [
    "from datetime", "import datetime", "def ", "print(", "datetime(",
    "strptime", "strftime", "timedelta", "for row in", "for item in",
    "if __name__", "class ", "return ", "None", "True", "False",
    "append(", "len(", "range(", "enumerate(",
  ];
  return response.replace(/```[\w]*\n?([\s\S]*?)```/g, (match, codeContent) => {
    const isProgramming = pythonKeywords.some(kw => codeContent.includes(kw));
    if (isProgramming) {
      return "";
    }
    return match;
  ...
  app.post("/api/agents/:id/sessions/:sessionId/messages", async (req: AuthenticatedRequest, res) => {
    try {
      ...
      try {
        ...
        detectedIntent = turnResult.intent;
          ...
          if (turnResult.nextAction !== 'generate_ai_response') {
            ...
            const suggestedActionPrefixes: Record<string, string> = {
              ...
              'Breakdown in details (L)': '[SYSTEM CONTEXT: The user clicked "Breakdown in details (L)". Provide a detailed step-by-step breakdown of the most recent expression. Use EXACTLY this format:\n\n**Expression:**\n<the full expression on its own line>\n\n**Step-by-Step:**\n1. <first sub-expression or argument>\n   → <brief plain-language explanation of what this step does>\n2. <second sub-expression or argument>\n   → <brief plain-language explanation of what this step does>\n... (continue numbering each distinct sub-expression)\nN. *Final:* <the full expression rewritten showing how all steps combine, referencing [Step 1],[Step 2],etc.>\n   → <one-sentence summary of what the complete expression produces>\n\nRules:\n- Break the expression into its smallest meaningful parts — each function call argument, each nested function, each literal string.\n- Each step MUST have a → annotation line immediately after it explaining what that step does in simple terms a non-technical user can understand.\n- For the Final step, show how all numbered steps combine back into the full expression. Use [Step N] references to show composition.\n- Do NOT include any additional headers or commentary beyond Expression and Step-by-Step.\n- Do NOT use code fences or markdown code blocks.\n- CRITICAL: Do NOT output Python, JavaScript, or any other programming language code. Use ONLY the custom expression language (e.g. If(), DateDiff(), And(), etc.) and plain English text.]\n\n',
              'Breakdown in details (S)': '[SYSTEM CONTEXT: The user clicked "Breakdown in details (S)". Provide a SHORT, high-level structural breakdown of the most recent expression. The goal is FEWER steps — show only the top-level arguments of the outermost function, NOT every nested sub-expression.\n\nHere is a CONCRETE EXAMPLE of the exact output format you must follow:\n\n**Expression:**\nIf(Eq(Status, "Active"), Add(Base, Bonus), 0)\n\n**Step-by-Step:**\n1. Eq(Status, "Active")\n   → This checks whether the employee status is "Active".\n2. Add(Base, Bonus)\n   → This adds the Base and Bonus amounts together.\n3. 0\n   → This is the default value when the condition is not met.\n4. If([Step 1], [Step 2], [Step 3])\n   → This returns the sum of Base and Bonus for active employees, or 0 otherwise.\n5. *Final:* If(Eq(Status, "Active"), Add(Base, Bonus), 0)\n\nNow apply this same format to the most recent expression. Use EXACTLY this structure:\n\n**Expression:**\n<the full expression on its own line>\n\n**Step-by-Step:**\n1. <first direct argument>\n   → <one short sentence explaining what this does>\n2. <second direct argument>\n   → <one short sentence explaining what this does>\n...\nN-1. <outermost function with [Step N] references>\n   → <one short sentence explaining how the steps combine>\nN. *Final:* <the full expression written out>\n\nCRITICAL RULES:\n- EVERY step MUST have a → annotation line immediately after it. NEVER omit the → line. If a step has no → line, your output is WRONG.\n- Keep each → annotation to one brief, plain-language sentence.\n- ONLY break down the outermost function into its direct arguments. Each argument becomes ONE step, no matter how complex that argument is internally.\n- Do NOT recursively decompose nested functions — leave them intact as a single step.\n- For nested If chains like If(cond1, val1, If(cond2, val2, default)), break down the OUTERMOST If only: Step 1 = cond1, Step 2 = val1, Step 3 = the entire inner If(...) as one step.\n- The composition step shows the outermost function structure using [Step N] references.\n- The Final step shows the complete expression (no → annotation needed for Final).\n- Do NOT include any additional headers or commentary beyond Expression and Step-by-Step.\n- Do NOT use code fences or markdown code blocks.\n- CRITICAL: Do NOT output Python, JavaScript, or any other programming language code. Use ONLY the custom expression language (e.g. If(), DateDiff(), And(), etc.) and plain English text.]\n\n',
              'Breakdown in details (MA)': '[SYSTEM CONTEXT: The user clicked "Breakdown in details (MA)". Your task is to EXPLAIN the most recent expression by showing how it was built up piece by piece — starting from the simplest building blocks and combining them into the full expression. This is an explanation of an existing expression — NOT a request to create a new one.\n\nHere is a CONCRETE EXAMPLE of the exact output format you must follow:\n\n**Expression:**\nIf(DateReHired != "", If(DateDiff(DateReHired, Today()) / 365.25 >= 5, Floor(DateDiff(DateReHired, Today()) / 365.25, 0), ""), If(DateDiff(DateHired, Today()) / 365.25 >= 5, Floor(DateDiff(DateHired, Today()) / 365.25, 0), ""))\n\n**Step-by-Step:**\n\n**Step A — The core years calculation:**\nWe start with the basic math: find the number of days between a given date and today, then divide by 365.25 to convert to years. The 365.25 accounts for leap years automatically.\nDateDiff(DateHired, Today()) / 365.25\n\n**Step B — Round down to whole years:**\nTenure is shown in complete years, not fractions. So we round the result from Step A down to the nearest whole number.\nFloor(DateDiff(DateHired, Today()) / 365.25, 0)\nThis says: "Drop any partial year — if someone has 6.9 years, show 6."\n\n**Step C — Should we even show a result? The 5-year threshold:**\nWe only want to display tenure for employees who have reached 5 or more years. Step B gives us the years — now we check whether that number clears the threshold.\nDateDiff(DateHired, Today()) / 365.25 >= 5\nIf yes, we return the Floor result from Step B. If no, we return a blank so the column stays empty for those employees.\n\n**Step D — The rehire check: which date should we use?**\nFor re-hired employees, counting from the original hire date would overcount their tenure. So the outermost question is: does a rehire date exist?\nDateReHired != ""\nIf yes, we route to the rehire branch. If no, we route to the original hire date branch.\n\n**Step E — The rehire branch: apply Steps B and C using the rehire date:**\nWhen an employee has a rehire date, we run the same threshold-and-floor logic from Steps B and C — but starting from DateReHired instead of DateHired.\nIf(DateDiff(DateReHired, Today()) / 365.25 >= 5, Floor(DateDiff(DateReHired, Today()) / 365.25, 0), "")\nThis says: "Show their tenure from the rehire date only if it reaches 5 years. Otherwise, show nothing."\n\n**Step F — The original hire branch: apply Steps B and C using the hire date:**\nFor employees who were never rehired, we run the identical logic against their original hire date.\nIf(DateDiff(DateHired, Today()) / 365.25 >= 5, Floor(DateDiff(DateHired, Today()) / 365.25, 0), "")\nThis says: "Show their tenure from the hire date only if it reaches 5 years. Otherwise, show nothing."\n\n**Step G — Wrap it all together:**\nStep D is the outer gate — it asks the rehire question and routes each employee to the right branch. Step E handles rehired employees; Step F handles everyone else.\nIf(DateReHired != "", ...Step E..., ...Step F...)\n\nNow apply this same format to the most recent expression. Use EXACTLY this structure for EVERY step:\n\n**Expression:**\n<copy the full expression here, exactly as it appeared>\n\n**Step-by-Step:**\n\n**Step A — <Short descriptive title>:**\n<One or two sentences explaining WHY this step exists and what it does — written for a non-technical user. Do NOT just restate the formula.>\n<the sub-expression for this step — no code fences, no backticks, just the raw expression text on its own line>\n<optional "This says:" line — use this when a brief plain-English translation of the sub-expression would help the user understand it>\n\n**Step B — <Short descriptive title>:**\n<Explanation — reference Step A by name if this step builds on it (e.g. "Plug Step A into..."). Explain the business reason, not just the mechanics.>\n<the sub-expression for this step>\n<optional "This says:" line>\n\n... (continue lettering through as many steps as the expression requires)\n\n**Step N — Wrap it all together:**\n<Explain how all prior steps combine. Reference earlier steps by letter. Describe which step acts as the outer gate and which are the branches.>\n<the full final expression using ...Step X... placeholders for readability>\n\nCRITICAL RULES:\n- Use letters (A, B, C...) not numbers.\n- Give each step a bold title that describes its purpose in plain English.\n- For EVERY step: write the explanation FIRST (before the sub-expression), then the sub-expression on its own line, then an optional "This says:" line if it adds clarity.\n- NEVER just label what the formula does mechanically — always explain the business reason (e.g. why we pick one date over another, why we check a threshold, why we round down).\n- When a step builds on a prior step, say so explicitly (e.g., "Plug Step B into Step A as the starting date").\n- Keep all explanations concise and non-technical — written for someone who understands the business logic but not the syntax.\n- Do NOT add any extra headers or commentary outside of the Step-by-Step format.\n- Do NOT use code fences or markdown code blocks.\n- This is an explanation of an existing expression. Do NOT apply expression-building validation rules to step labels.\n- CRITICAL: Do NOT output Python, JavaScript, or any other programming language code. Use ONLY the custom expression language (e.g. If(), DateDiff(), Floor(), etc.) and plain English text.]\n\n',
            ...
            responseContent = stripProgrammingCode(validation.recoveryResponse);
        ...
        const isKeyError = aiError.message?.includes("GEMINI_API_KEY");
        responseContent = isRateLimit
          ? "The AI service is temporarily busy. Please wait a moment and try again."
          : isKeyError
          ? "I apologize, but I'm having trouble generating a response. The Gemini API key may not be configured correctly."
