        ...
        detectedIntent = turnResult.intent;
          ...
          if (turnResult.nextAction !== 'generate_ai_response') {
            ...
            const suggestedActionPrefixes: Record<string, string> = {
              ...
              'Breakdown in details (MA)': '[SYSTEM CONTEXT: The user clicked "Breakdown in details (MA)". Show how the expression is constructed from its smallest building blocks up to the final form — each step adds exactly one new piece. Use EXACTLY this format:\n\n**Expression:**\n<the full expression on its own line>\n\n**Step-by-Step:**\n1. <the innermost value, column reference, or simplest sub-expression>\n   → <one sentence — what this piece represents, in payroll/HR business terms>\n2. <previous step wrapped or combined with the next piece>\n   → <one sentence — what this added piece does and why it is needed>\n... (continue, each step adding exactly one new layer or function)\nN. *Final:* <the full expression written out>\n   → <one sentence — what the complete expression produces as a result>\n\nCRITICAL RULES:\n- Each step MUST show the expression growing — start from the innermost piece and work outward. Every step\'s expression should be a valid sub-expression or partial expression that includes everything from the previous step plus one new piece.\n- EVERY step MUST have a → annotation line. NEVER omit it.\n- Tone is professional and direct — written for a payroll administrator. Avoid phrases like "Now let\'s look at", "Think of it as", "Simply put", "This tells the system". Write annotations as plain business-language statements that a payroll professional would use with a colleague.\n- Keep each annotation to one sentence. Focus on the business reason or data logic behind the piece, not the technical mechanics.\n- If the expression includes a safety guard (e.g., If check for blank or zero), introduce it as one of the steps and explain the business scenario it protects against.\n- Do NOT include any additional headers or commentary beyond Expression and Step-by-Step.\n- Do NOT use code fences or markdown code blocks.]\n\n',
