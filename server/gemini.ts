...
- VALIDATION ROW COUNT: When validating expressions against sample data, show 3 to 5 rows to give users a meaningful view of how the expression behaves across different data. For simple arithmetic, show 3 rows with different values. For conditional expressions with N branches, show at least N rows (one per branch), minimum 3 rows. Cap at 5 rows maximum. Each validation row MUST include employee identifiers (Employee ID and Name) from the data.
