...
- **Welcome Screen Step (Step 10)**: Added a new "Welcome Screen" step to the wizard (now 10 steps total). This step auto-generates a greeting message and 4-6 suggested prompts using AI based on the agent's business use case and domain knowledge. Users can toggle the welcome screen on/off, edit the greeting/prompts, add/remove prompts, and regenerate with AI. The welcome screen is displayed on the chat page when there are no messages yet, and clicking a suggested prompt immediately sends it. Existing agents auto-generate a welcome config on first chat load. Data stored as `welcomeConfig` on the Agent schema.
