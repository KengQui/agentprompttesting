...
import { PersonalSSTurnManager } from './turn-manager';
import { PersonalSSFlowController } from './flow-controller';
import type { ClassificationResult, TurnResult, AgentConfig } from '../../../server/components/types';

export class PersonalSSOrchestrator extends BaseOrchestrator {
  constructor(agentConfig: AgentConfig) {
    ...
    this.turnManager = new PersonalSSTurnManager();
    this.flowController = new PersonalSSFlowController();
  }
...
export function createOrchestrator(agentConfig: AgentConfig): PersonalSSOrchestrator {
  return new PersonalSSOrchestrator(agentConfig);
}
