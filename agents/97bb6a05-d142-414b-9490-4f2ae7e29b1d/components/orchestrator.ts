...
import { Lifeevent5TurnManager } from './turn-manager';
import { Lifeevent5FlowController } from './flow-controller';
import type { ClassificationResult, TurnResult, AgentConfig } from '../../../server/components/types';

export class Lifeevent5Orchestrator extends BaseOrchestrator {
  constructor(agentConfig: AgentConfig) {
    ...
    this.turnManager = new Lifeevent5TurnManager();
    this.flowController = new Lifeevent5FlowController();
  }
...
export function createOrchestrator(agentConfig: AgentConfig): Lifeevent5Orchestrator {
  return new Lifeevent5Orchestrator(agentConfig);
}
