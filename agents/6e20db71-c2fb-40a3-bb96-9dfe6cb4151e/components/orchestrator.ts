...
import { TestAgentTurnManager } from './turn-manager';
import { TestAgentFlowController } from './flow-controller';
import type { ClassificationResult, TurnResult, AgentConfig } from '../../../server/components/types';

export class TestAgentOrchestrator extends BaseOrchestrator {
  constructor(agentConfig: AgentConfig) {
    ...
    this.turnManager = new TestAgentTurnManager();
    this.flowController = new TestAgentFlowController();
  }
...
export function createOrchestrator(agentConfig: AgentConfig): TestAgentOrchestrator {
  return new TestAgentOrchestrator(agentConfig);
}
